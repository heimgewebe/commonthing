...
class StagingCellRuntimeContractTests(unittest.TestCase):
    ...
    def test_legacy_migration_is_read_only_historical_state(self) -> None:
        subparsers = next(
            action
            for action in staging.parser()._actions
            if isinstance(action, argparse._SubParsersAction)
        )
        self.assertNotIn("migrate-legacy-state", subparsers.choices)
        self.assertTrue(hasattr(staging, "load_legacy_state_migration"))
        self.assertFalse(hasattr(staging, "command_migrate_legacy_state"))
